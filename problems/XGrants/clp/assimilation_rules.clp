...
(defrule ASSIMILATION2::modify-overlap
	?sub <-(ASSIMILATION2::UPDATE-OVERLAP (parameter ?p) (overlap-time# ?tr))
	?meas <- (REQUIREMENTS::Measurement (Parameter ?p) (Opt-Alt-Coincidence# nil) (Coverage ?region)(factHistory ?fh))
	=>
	(modify ?meas (Opt-Alt-Coincidence# ?tr) (factHistory (str-cat "{R" (?*rulesMap* get ASSIMILATION2::modify-overlap) " " ?fh " S" (call ?sub getFactId) "}")))
)

(defrule ASSIMILATION2::modify-temporal-resolution2
	?sub <-(ASSIMILATION2::UPDATE-REV-TIME2 (parameter ?p) (avg-revisit-time-global# ?new-time)
	(avg-revisit-time-US# ?new-time-us))
	?meas <- (REQUIREMENTS::Measurement (Parameter ?p) (Temporal-resolution# nil) (Temporal-resolution nil) (Coverage ?region)(factHistory ?fh))
	=>
	(if (eq ?region Global) then (bind ?tr ?new-time) else (bind ?tr ?new-time-us))
	(modify ?meas (Temporal-resolution# ?tr) (Opt-Alt-Coincidence# ?tr) (avg-revisit-time-global# ?new-time) (avg-revisit-time-US# (adapt-GEO-revisit ?new-time-us))(factHistory (str-cat "{R" (?*rulesMap* get ASSIMILATION2::modify-temporal-resolution2) " " ?fh " S" (call ?sub getFactId) "}")))
)
