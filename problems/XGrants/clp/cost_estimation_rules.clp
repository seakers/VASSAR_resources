...
(defrule COST-ESTIMATION::estimate-instrument-cost
    ...
    ?instr <- (CAPABILITIES::Manifested-instrument (Name ?name) (cost# nil) (mass# ?m&~nil&:(> ?m 12)) (dimension-x# ?m&~nil&:(> ?m 0.3)) (average-power# ?p&~nil) (average-data-rate# ?rb&~nil)
...
(defrule COST-ESTIMATION::estimate-bus-non-recurring-cost
    ...
    ?miss <- (MANIFEST::Mission (bus-non-recurring-cost# nil)
        ...
        (ADCS-mass# ?adcm&~nil) (standard-bus ?bus) (satellite-dry-mass ?m&~nil&:(> ?m 12)) (payload-dimensions ?pdims&~nil&:(> (nth$ 1 ?pdims) 0.3))
        )
  ...
  (deffunction str-cost-non-recurring (?strm)
    (return (* 646 (** ?strm 0.684)))
  )

  (deffunction prop-cost-non-recurring (?prm) ; TODO use RCS tank volume
  ...
  (deffunction comm-cost-non-recurring (?comm) ; not in existence
  ...
  (deffunction therm-cost-non-recurring (?thm)
    (return (* 646 (** ?thm 0.684)))
  )

  (deffunction eps-cost-non-recurring (?epsm)
    (return (* 64.3 ?epsm))
  )
...
(defrule COST-ESTIMATION::estimate-bus-TFU-recurring-cost
    ...
    (deffunction adcs-cost-recurring (?adcm)
      (return (* 795 (** ?adcm 0.593)))
    )

    (deffunction comm-cost-recurring (?comm)
      (return (* 883.7 (** ?comm 0.491)))
    )
...
(defrule COST-ESTIMATION::estimate-integration-and-testing-cost
    ...
    ?miss <- (MANIFEST::Mission (IAT-non-recurring-cost# nil) (IAT-recurring-cost# nil) (IAT-cost# nil)
        (spacecraft-non-recurring-cost# ?scnr&~nil) (spacecraft-recurring-cost# ?scr&~nil) (satellite-dry-mass ?m&~nil&:(> ?m 12)) (payload-dimensions ?pdims&~nil&:(> (nth$ 1 ?pdims) 0.3))
        )
    =>
...
(defrule COST-ESTIMATION::estimate-operations-cost-std
    ...
    ?miss <- (MANIFEST::Mission (satellite-cost# ?sat&~nil) (operations-cost# nil)
        ...
        (sat-data-rate-per-orbit# ?rbo&nil) (satellite-dry-mass ?m&~nil&:(> ?m 12)) (payload-dimensions ?pdims&~nil&:(> (nth$ 1 ?pdims) 0.3)))
    =>
...
(defrule COST-ESTIMATION::estimate-operations-cost-cubesat
    ...
    (declare (salience -5))
    ?miss <- (MANIFEST::Mission (satellite-cost# ?sat&~nil) (operations-cost# nil)
        (lifetime ?life &~nil) (num-of-planes# ?np&~nil) (num-of-sats-per-plane# ?ns&~nil) (satellite-dry-mass ?m&~nil&:(<= ?m 12)) (payload-dimensions ?pdims&~nil&:(<= (nth$ 1 ?pdims) 0.3)))
    =>
    ;(bind ?N (* ?np ?ns))
...
(defrule COST-ESTIMATION::launch-cost-cubesat-override
    ...
    (declare (salience 10))
    ?miss <- (MANIFEST::Mission (launch-cost# ?launch&~nil) (num-of-planes# ?np&~nil) (num-of-sats-per-plane# ?ns&~nil) (satellite-dry-mass ?m&~nil&:(<= ?m 12)) (payload-dimensions ?pdims&~nil&:(<= (nth$ 1 ?pdims) 0.3)))
    =>
    (bind ?N (* ?np ?ns))
    (bind ?cost (* (* ?N 0.1) 12))
...
(defrule INFLATION::inflate-all-cost-values
    ...
    =>
    (bind ?miss-r (MatlabFunctions inflate ?miss-r 2010 ?ld))
    ...
    (bind ?iat (MatlabFunctions inflate ?iat 2010 ?ld))
    (bind ?misscost (MatlabFunctions inflate ?misscost 2010 ?ld))
    (bind ?pay (MatlabFunctions inflate ?pay 2010 ?ld))
    (bind ?sat (MatlabFunctions inflate ?sat 2010 ?ld))
    (bind ?str (MatlabFunctions inflate ?str 2010 ?ld))
    (bind ?prop (MatlabFunctions inflate ?prop 2010 ?ld))
    (bind ?adcs (MatlabFunctions inflate ?adcs 2010 ?ld))
    (bind ?comm (MatlabFunctions inflate ?comm 2010 ?ld))
    (bind ?therm (MatlabFunctions inflate ?therm 2010 ?ld))
    (bind ?eps (MatlabFunctions inflate ?eps 2010 ?ld))
