...
(defrule MANIFEST::assign-injection-time
    ...
    =>
    (modify ?miss (injection-time Medium))
    )
...
(defrule MASS-BUDGET::electric-propulsion-thrust-requirement-LEO
    ...
    ?miss <- (MANIFEST::Mission (injection-time ?time&~nil) (delta-V ?dV&~nil) (orbit-type ~GEO) (orbit-altitude# ?alt&~nil) (Propulsion-Injection Electric) (thrust-requirement nil))
    =>
    (bind ?dV-error (- (sqrt (/ 3.986e14 (+ ?alt 10))) (sqrt (/ 698600.5 ?alt))))
    (if (eq ?time Low) then (modify ?miss (thrust-requirement (ep-thrust-requirement 365 ?dV-error)))
     elif (eq ?time Medium) then (modify ?miss (thrust-requirement (ep-thrust-requirement 100 ?dV-error)))
...
(deffunction ep-thrust-requirement (?transfer-time ?dV)
    return (/ ?dV (* ?transfer-time 24 60 60))
    )